const SQLV5_SEARCH_INDEX = [
  ['Home','index.html','SQL v5 gateway for ClassDesk, QueryFlow v3, QueryPilot v9, HMG ecosystem and deployment.','home gateway overview'],
  ...
  ['About','about.html','About SQL v5 and feature explanations.','about features hmg'],
...
(function(){
  ...
  function render(q=''){
    ...
    let rows=SQLV5_SEARCH_INDEX.map(x=>({x,score: words.reduce((a,w)=>a+(x.join(' ').toLowerCase().includes(w)?1:0),0)}));
